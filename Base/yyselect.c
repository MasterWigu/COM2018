...
/* 26 */	"expr: ATR(expr,LOCAL)",
/* 27 */	"expr: ATR(expr,ID)",
/* 28 */	"assign: expr",
/* 29 */	"expr: ATR(assign,lvalue)",
/* 30 */	"expr: ID",
/* 31 */	"base: ALLOC(alloc,lvalue)",
/* 32 */	"alloc: expr",
/* 33 */	"expr: INT",
/* 34 */	"expr: REAL",
/* 35 */	"expr: STR",
/* 36 */	"expr: CALL(ID,args)",
/* 37 */	"expr: ADD(expr,expr)",
/* 38 */	"expr: SUB(expr,expr)",
/* 39 */	"expr: MUL(expr,expr)",
/* 40 */	"expr: DIV(expr,expr)",
/* 41 */	"expr: FACT(expr)",
/* 42 */	"expr: MOD(expr,expr)",
/* 43 */	"expr: EQ(expr,expr)",
/* 44 */	"expr: NE(expr,expr)",
/* 45 */	"expr: LT(expr,expr)",
/* 46 */	"expr: LE(expr,expr)",
/* 47 */	"expr: GT(expr,expr)",
/* 48 */	"expr: GE(expr,expr)",
/* 49 */	"expr: UMINUS(expr)",
/* 50 */	"expr: NOT(expr)",
/* 51 */	"expr: AND(and,expr)",
/* 52 */	"and: expr",
/* 53 */	"expr: OR(or,expr)",
/* 54 */	"or: expr",
/* 55 */	"base: ELSE(if,base)",
/* 56 */	"if: IF(cond,base)",
/* 57 */	"base: IF(cond,base)",
/* 58 */	"cond: expr",
/* 59 */	"base: WHILE(do,expr)",
/* 60 */	"do: DO(begin,stmt)",
/* 61 */	"begin: START",
/* 62 */	"base: SC(expr,fbase)",
/* 63 */	"fbase: FOR(infor,flist)",
/* 64 */	"infor: IN(fbegin,expr)",
/* 65 */	"flist: SC(base,expr)",
/* 66 */	"fbegin: START",
/* 67 */	"brk: BREAK",
/* 68 */	"brk: CONTINUE",
/* 69 */	"expr: INCR(lvalue)",
/* 70 */	"expr: DECR(lvalue)",
/* 71 */	"expr: POSINC(lvalue)",
/* 72 */	"expr: POSDEC(lvalue)",
};
...
static void yyclosure_expr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 58, c + 1, p->cost[yycond_NT]);
	if (c + 1 < p->cost[yycond_NT]) {
		p->cost[yycond_NT] = c + 1;
		p->rule.yycond = 1;
	}
	yytrace(a, 54, c + 1, p->cost[yyor_NT]);
	if (c + 1 < p->cost[yyor_NT]) {
		p->cost[yyor_NT] = c + 1;
		p->rule.yyor = 1;
	}
	yytrace(a, 52, c + 1, p->cost[yyand_NT]);
	if (c + 1 < p->cost[yyand_NT]) {
		p->cost[yyand_NT] = c + 1;
		p->rule.yyand = 1;
	}
	yytrace(a, 32, c + 1, p->cost[yyalloc_NT]);
	if (c + 1 < p->cost[yyalloc_NT]) {
		p->cost[yyalloc_NT] = c + 1;
		p->rule.yyalloc = 1;
	}
	yytrace(a, 28, c + 1, p->cost[yyassign_NT]);
...
static void yylabel(NODEPTR_TYPE a, NODEPTR_TYPE u) {
	...
	case 59: /* SC */
		...
		yytrace(a, 62, c + 0, p->cost[yybase_NT]);
		if (c + 0 < p->cost[yybase_NT]) {
			p->cost[yybase_NT] = c + 0;
			p->rule.yybase = 8;
			yyclosure_base(a, c + 0);
		...
		yytrace(a, 65, c + 0, p->cost[yyflist_NT]);
	...
	case 259: /* ID */
		...
		/* expr: ID */
		yytrace(a, 30, 1 + 0, p->cost[yyexpr_NT]);
		if (1 + 0 < p->cost[yyexpr_NT]) {
			p->cost[yyexpr_NT] = 1 + 0;
			p->rule.yyexpr = 6;
			yyclosure_expr(a, 1 + 0);
	...
	case 261: /* DO */
		...
		yytrace(a, 60, c + 0, p->cost[yydo_NT]);
	...
	case 262: /* WHILE */
		...
		yytrace(a, 59, c + 0, p->cost[yybase_NT]);
		if (c + 0 < p->cost[yybase_NT]) {
			p->cost[yybase_NT] = c + 0;
			p->rule.yybase = 7;
			yyclosure_base(a, c + 0);
	...
	case 263: /* IF */
		...
		yytrace(a, 56, c + 0, p->cost[yyif_NT]);
	...
	case 265: /* FOR */
		...
		yytrace(a, 63, c + 0, p->cost[yyfbase_NT]);
	...
	case 266: /* IN */
		...
		yytrace(a, 64, c + 0, p->cost[yyinfor_NT]);
	...
	case 270: /* BREAK */
		/* brk: BREAK */
		yytrace(a, 67, 1 + 0, p->cost[yybrk_NT]);
	...
	case 294: /* START */
		/* begin: START */
		yytrace(a, 61, 1 + 0, p->cost[yybegin_NT]);
		if (1 + 0 < p->cost[yybegin_NT]) {
			p->cost[yybegin_NT] = 1 + 0;
			p->rule.yybegin = 1;
		}
		/* fbegin: START */
		yytrace(a, 66, 1 + 0, p->cost[yyfbegin_NT]);
...
static void yykids(NODEPTR_TYPE p, int eruleno, NODEPTR_TYPE kids[]) {
	...
	switch (eruleno) {
	case 68: /* brk: CONTINUE */
	case 67: /* brk: BREAK */
	case 66: /* fbegin: START */
	case 61: /* begin: START */
	case 35: /* expr: STR */
	case 34: /* expr: REAL */
	case 33: /* expr: INT */
	...
	case 1: /* list: NIL */
		break;
	case 65: /* flist: SC(base,expr) */
	case 64: /* infor: IN(fbegin,expr) */
	case 63: /* fbase: FOR(infor,flist) */
	case 62: /* base: SC(expr,fbase) */
	case 60: /* do: DO(begin,stmt) */
	case 59: /* base: WHILE(do,expr) */
	case 57: /* base: IF(cond,base) */
	case 56: /* if: IF(cond,base) */
	case 55: /* base: ELSE(if,base) */
	case 53: /* expr: OR(or,expr) */
	case 51: /* expr: AND(and,expr) */
	case 48: /* expr: GE(expr,expr) */
	case 47: /* expr: GT(expr,expr) */
	case 46: /* expr: LE(expr,expr) */
	case 45: /* expr: LT(expr,expr) */
	case 44: /* expr: NE(expr,expr) */
	case 43: /* expr: EQ(expr,expr) */
	case 42: /* expr: MOD(expr,expr) */
	case 40: /* expr: DIV(expr,expr) */
	case 39: /* expr: MUL(expr,expr) */
	case 38: /* expr: SUB(expr,expr) */
	case 37: /* expr: ADD(expr,expr) */
	case 31: /* base: ALLOC(alloc,lvalue) */
	case 29: /* expr: ATR(assign,lvalue) */
	...
	case 2: /* list: SC(list,base) */
		...
		break;
	case 72: /* expr: POSDEC(lvalue) */
	case 71: /* expr: POSINC(lvalue) */
	case 70: /* expr: DECR(lvalue) */
	case 69: /* expr: INCR(lvalue) */
	case 50: /* expr: NOT(expr) */
	case 49: /* expr: UMINUS(expr) */
	case 41: /* expr: FACT(expr) */
	case 27: /* expr: ATR(expr,ID) */
	case 26: /* expr: ATR(expr,LOCAL) */
...
{ fprintf(outfp, pfLOAD); }
	...
	case 24: /* lvalue: INDEX(lvec,expr) */
#line 88 "diy.brg"
{ fprintf(outfp, pfIMM pfMUL pfADD, p->info == 3 ? 2*pfWORD : pfWORD);}
		break;
	case 25: /* lvalue: PTR(lvalue) */
#line 89 "diy.brg"
{ fprintf(outfp, LEFT_CHILD(p)->info == 3  ? pfLOAD2 : pfLOAD); }
	...
	case 26: /* expr: ATR(expr,LOCAL) */
#line 92 "diy.brg"
{ doATR(p); if (RIGHT_CHILD(p)->info != 3) fprintf(outfp, pfDUP pfLOCA, RIGHT_CHILD(p)->value.i * (pfWORD/4)); else fprintf(outfp, pfDUP2 pfLOCAL pfSTORE2, RIGHT_CHILD(p)->value.i * (pfWORD/4)); }
		break;
	case 27: /* expr: ATR(expr,ID) */
#line 93 "diy.brg"
{ doATR(p); if (RIGHT_CHILD(p)->info != 3) fprintf(outfp, pfDUP pfADDRA, RIGHT_CHILD(p)->value.s); else fprintf(outfp, pfDUP2 pfADDR pfSTORE2, RIGHT_CHILD(p)->value.s);  }
		break;
	case 28: /* assign: expr */
#line 95 "diy.brg"
{ fprintf(outfp, pfDUP); }
	...
	case 29: /* expr: ATR(assign,lvalue) */
#line 96 "diy.brg"
{ doATR2(p); fprintf(outfp, RIGHT_CHILD(p)->info != 3 ? pfSTORE : pfSTORE2); }
		break;
	case 30: /* expr: ID */
#line 97 "diy.brg"
