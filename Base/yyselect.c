/*
generated at Fri May 24 15:19:44 2019
...
{ fprintf(outfp, pfLOAD); }
	...
	case 24: /* lvalue: INDEX(lvec,expr) */
#line 89 "diy.brg"
{ if(LEFT_CHILD(p)->info==3)fprintf(outfp, pfIMM pfMUL pfADD, 2*pfWORD); else if (LEFT_CHILD(p)->info == 2) fprintf(outfp, pfADD); else fprintf(outfp, pfIMM pfMUL pfADD, pfWORD);}
		break;
	case 25: /* lvalue: PTR(lvalue) */
#line 90 "diy.brg"
...
{ fprintf(outfp, pfDUP); }
	...
	case 27: /* expr: ATR(assign,lvalue) */
#line 97 "diy.brg"
{ doATR2(p); if (RIGHT_CHILD(p)->attrib == '[' && LEFT_CHILD(RIGHT_CHILD(p))->info == 2)fprintf(outfp, pfSTCHR); else if (RIGHT_CHILD(p)->info == 3 )fprintf(outfp, pfSTORE2); else fprintf(outfp, pfSTORE); }
		break;
	case 28: /* expr: ID */
#line 98 "diy.brg"
...
{ fprintf(outfp, pfDATA pfALIGN pfLABEL pfDOUBLE, mklbl(++lbl), p->value.r); fprintf(outfp, pfTEXT pfALIGN pfADDR pfLOAD2, mklbl(lbl)); }
	...
	case 33: /* expr: STR */
#line 106 "diy.brg"
{ lbl++; fprintf(outfp, pfDATA pfALIGN pfLABEL, mklbl(lbl)); outstr(p->value.s); fprintf(outfp, pfTEXT pfADDR, mklbl(lbl)); }
		break;
	case 34: /* expr: CALL(ID,args) */
#line 107 "diy.brg"
